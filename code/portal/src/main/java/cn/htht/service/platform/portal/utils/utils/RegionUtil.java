...
public class RegionUtil {
    ...
    /**
     * 获取到所选区域点中最大经纬度和最小经纬度
     *
     * @param longtiude
     * @param latitude
     * @return
     */
    public static Map<String, Double> getRegionRange(double[] longtiude, double[] latitude) {
        Map<String, Double> regionMap = new HashMap<>();
        regionMap.put("maxLon", Arrays.stream(longtiude).filter(Objects::nonNull).max().getAsDouble());
        regionMap.put("maxLat", Arrays.stream(latitude).filter(Objects::nonNull).max().getAsDouble());
        regionMap.put("minLon", Arrays.stream(longtiude).filter(Objects::nonNull).min().getAsDouble());
        regionMap.put("minLat", Arrays.stream(latitude).filter(Objects::nonNull).min().getAsDouble());
        return regionMap;
    }
}
