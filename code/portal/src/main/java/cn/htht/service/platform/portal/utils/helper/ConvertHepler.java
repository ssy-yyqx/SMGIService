...
public final class ConvertHepler {
    ...
    public static final String bytesToHexString(byte[] bytes) {
        ...
        for (byte b : bytes) {
            //字节高4位
            result.append(String.valueOf(BaseConstant.HEX_CHAR_STR.charAt((b & 0xF0) >> 4)));
            //字节低4位
            result.append(String.valueOf(BaseConstant.HEX_CHAR_STR.charAt(b & 0x0F)));
            result.append(" ");
        }
        return result.toString();
    }
    ...
    private static final int toByte(char c) {
        return (byte) BaseConstant.HEX_CHAR_STR.indexOf(c);
    }
