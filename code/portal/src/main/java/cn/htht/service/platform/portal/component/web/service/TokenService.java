...
public class TokenService {
    ...
    static { // 将证书文件里边的私钥公钥拿出来
        try {
            InputStream inputStream = Thread.currentThread().getContextClassLoader().getResourceAsStream("smgi.jks"); // 寻找证书文件
            KeyStore keyStore = KeyStore.getInstance("JKS"); // java key store 固定常量
            keyStore.load(inputStream, "smgi2021".toCharArray());
            privateKey = (PrivateKey) keyStore.getKey("smgi-jwt", "smgi2021".toCharArray()); // jwt 为 命令生成整数文件时的别名
            publicKey = keyStore.getCertificate("smgi-jwt").getPublicKey();
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    public LoginUser getLoginUser(HttpServletRequest request) {
        ...
        if (StringUtils.isNotEmpty(token)) {
            Claims claims = parseToken(token, type.equals("rsa"));
            // 解析对应的权限以及用户信息
    ...
    public String createToken(LoginUser loginUser) {
        ...
        return createToken(claims, type.equals("rsa"));
    }
     ...
     * @return 令牌
     */
    private String createToken(Map<String, Object> claims, boolean isRsa) {
        if (isRsa) {
            return Jwts.builder()
                    .setClaims(claims)
                    .signWith(SignatureAlgorithm.RS256, privateKey) // 使用公钥私钥
                    .compact();
        } else {
            return Jwts.builder()
                    .setClaims(claims)
     ...
     * @return 数据声明
     */
    private Claims parseToken(String token, boolean isRsa) {
        if (isRsa) {
            return Jwts.parser()
